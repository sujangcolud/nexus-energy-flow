...
BEGIN
    ...
    -- Get withdrawals data for the date (using 'amount' column)
    -- Note: withdrawals table doesn't have withdrawal_source in schema, so we'll use different logic
    SELECT 
        COALESCE(COUNT(*), 0) as total_count,
        COALESCE(SUM(amount), 0) as total_amount,
        -- For withdrawals, we'll categorize based on purpose or use simple totals
        COALESCE(SUM(CASE WHEN purpose ILIKE '%cash%' THEN amount ELSE 0 END), 0) as cash_total,
        COALESCE(SUM(CASE WHEN purpose ILIKE '%bank%' THEN amount ELSE 0 END), 0) as bank_total,
        COALESCE(SUM(CASE WHEN purpose ILIKE '%cooperative%' OR purpose ILIKE '%saving%' THEN amount ELSE 0 END), 0) as cooperative_total
    INTO withdrawals_data
    FROM withdrawals 
    WHERE DATE(created_at) = target_date;

    -- Get cooperative savings data for the date (using 'contribution_amount' column)
    -- Note: cooperative_savings doesn't have payment_method, so we'll assume cash
    SELECT 
        COALESCE(COUNT(*), 0) as total_count,
        COALESCE(SUM(contribution_amount), 0) as total_amount,
        COALESCE(SUM(contribution_amount), 0) as cash_total, -- Assume all cash
        0 as esewa_total,
        0 as fonepay_total
    INTO savings_data
    ...
    -- Calculate running balances (simplified calculation)
    SELECT 
        ...
        COALESCE((orders_data.esewa_total + charging_data.esewa_total + deposits_data.esewa_total) - 
                 (expenses_data.esewa_total), 0) as esewa_balance,
        COALESCE((orders_data.fonepay_total + charging_data.fonepay_total + deposits_data.fonepay_total) - 
                 (expenses_data.fonepay_total), 0) as fonepay_balance,
