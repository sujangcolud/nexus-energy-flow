...
const OrdersTab = () => {
  ...
  return (
    <div className="space-y-6">
      {" "}
      {/* Removed top padding pt-4 md:pt-6 */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          <Card className="shadow-sm">
            {" "}
            {/* Added subtle shadow to menu card container */}
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center gap-2 mb-6 flex-wrap pb-2">
                {" "}
                {/* Increased mb, added pb for spacing */}
                ...
                {productCategories.map((category) => (
                  <Button
                    ...
                    } // Changed outline to secondary for inactive
                    size="sm"
                          ...
                          >
                            <CardContent className="p-3 flex flex-col flex-grow">
                              ...
                              <div className="flex justify-between items-start mb-1.5">
                                {" "}
                                {/* Reduced mb */}
                                <h4 className="font-semibold text-sm text-gray-800 dark:text-gray-200 group-hover:text-primary transition-colors line-clamp-2">
                                  {" "}
                                  {/* Adjusted text size, color, line-clamp */}
                                  {item.name}
                                </h4>
                                <Badge
                                  variant="outline"
                                ...
                                {/* Currency updated, adjusted badge style & padding */}
                              </div>
                              {item.description && (
                                <p className="text-xs text-gray-500 dark:text-gray-400 mb-2 line-clamp-1 flex-grow">
                                  {" "}
                                  {/* Adjusted text size, color, mb, line-clamp */}
                                  {item.description}
                                </p>
                              )}
                              {!item.description && (
            ...
            <CardHeader className="bg-muted/30 dark:bg-muted/50 border-b border-border/50">
              <CardTitle className="flex items-center justify-between text-lg">
                ...
                {cart.length > 0 && (
                  ...
                  >
                    <Trash2 className="h-4 w-4 mr-1.5 group-hover:animate-shake" />{" "}
                    Clear Cart
                  </Button>
                )}
              </CardTitle>
                ...
                <>
                  <div className="space-y-2.5 max-h-[280px] md:max-h-[320px] overflow-y-auto pr-1 simple-scrollbar">
                    {cart.map((item) => (
                      <div
                        key={item.id}
                        className="flex items-center justify-between p-3 bg-background hover:bg-muted/30 dark:hover:bg-muted/20 rounded-lg border border-border/70 hover:border-primary/30 transition-all duration-150 ease-in-out group"
                      >
              ...
              <Table className="min-w-full">
                ...
                <TableHeader>
                  <TableRow className="bg-muted/50 dark:bg-muted/30">
                    ...
                    <TableHead className="py-2.5 px-4 text-xs font-medium text-muted-foreground whitespace-nowrap w-[200px]">
                      Item
                    </TableHead>
                    <TableHead className="py-2.5 px-4 text-xs font-medium text-muted-foreground whitespace-nowrap text-center">
                      Quantity
                    </TableHead>
                    <TableHead className="py-2.5 px-4 text-xs font-medium text-muted-foreground whitespace-nowrap text-right">
                      Rate
                    </TableHead>
                    <TableHead className="py-2.5 px-4 text-xs font-medium text-muted-foreground whitespace-nowrap text-right">
                      Total
                ...
                <TableBody>
                  {orders.map((order) => (
                    <TableRow
                      key={order.id}
                      className="hover:bg-muted/20 dark:hover:bg-muted/10 transition-colors border-b border-border/30 last:border-b-0"
                    >
                      <TableCell className="py-2.5 px-4 text-sm whitespace-nowrap">
                        {new Date(order.order_date).toLocaleDateString()}
                      </TableCell>
                      <TableCell className="py-2.5 px-4 text-sm font-medium whitespace-normal break-words w-[200px]">
                        {order.item_name}
                      </TableCell>
                      <TableCell className="py-2.5 px-4 text-sm text-center whitespace-nowrap">
                        {order.quantity}
                      </TableCell>
                      <TableCell className="py-2.5 px-4 text-sm text-right whitespace-nowrap">
