-- Add missing columns to daily_summary for the new Verification System
...
CREATE TABLE IF NOT EXISTS public.verification_settings (
    ...
    cutoff_date DATE DEFAULT '2024-01-01',
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

-- Initialize with one row
INSERT INTO public.verification_settings (id, opening_cash_balance, cutoff_date)
VALUES (1, 0, '2024-01-01')
ON CONFLICT (id) DO NOTHING;

-- Update update_daily_summary to populate new columns and follow the specific Cash in Hand logic
-- Update sync_daily_summary_for_date to populate new columns and follow the specific Cash in Hand logic
...
BEGIN
    ...
    -- SPECIFIC REQUIREMENT for Verification System:
    -- Cash Variance Calculation per day (without opening balance which is handled in frontend/reports)
    -- Formula: (Cash Orders + Cash Charging) - cash expenses - cash withdrawal from counter - cash savings - cash deposits
    v_system_cash_calc := (v_total_income_cash_orders + v_total_income_cash_charging) - v_total_expenses_cash - v_total_withdrawals_cash - v_total_savings_cash - v_total_deposits_from_cash;
