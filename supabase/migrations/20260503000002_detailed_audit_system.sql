-- Advanced Business Intelligence View with Robust Date Handling & Detailed Audits
...
allocated_costs AS (
  SELECT
    ...
    CASE
      ...
      -- If no food sales but food costs exist, we can't distribute proportionally by sales.
      -- We'll just assign the cost to 'Food (Veg/General)' as a fallback
...
-- AI Auditor: Detailed Detection View
...
-- Weekly Cash Audit (High Confidence only)
SELECT
  business_date,
  category_group,
  category_items,
