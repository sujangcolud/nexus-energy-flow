...
-- Correct logical errors in Daily Summary and Ledger calculations
-- This migration ensures accounting consistency across all summary functions.
...
DECLARE
    ...
    -- Deposits (Money sent from various wallets to Bank/Esewa/etc.)
    v_total_deposits NUMERIC;
    v_total_deposits_cash NUMERIC;
    v_total_deposits_esewa NUMERIC;
    v_total_deposits_from_cash NUMERIC; -- Deposits MADE FROM cash

    -- Savings (Money received from members - Inflow)
    v_total_savings NUMERIC;
    v_total_savings_cash NUMERIC;
    v_total_savings_fonepay NUMERIC;
    v_total_savings_esewa NUMERIC;

    -- Withdrawals (Money paid out - Outflow)
...
BEGIN
    ...
    -- 4. Calculate deposits (Moving money from wallets to destination)
    -- total_deposits_cash here means "Money deposited TO Cash" (Inflow to cash, but rarely used this way)
    -- total_deposits_from_cash means "Money deposited FROM Cash counter to Bank/Esewa" (Outflow from cash)
    SELECT
        COALESCE(SUM(amount), 0),
    ...
    -- 5. Calculate savings (Money received from members - Inflow)
    SELECT
    ...
    -- 6. Calculate withdrawals (Money paid out - Outflow)
    SELECT
        COALESCE(SUM(amount), 0),
    ...
    -- 7. Calculate ledger balances (Net Daily Changes)
    -- Cash: (Sales In + Savings In + Deposits In) - (Expenses Out + Withdrawals Out + Deposits From Counter Out)
    v_cash_balance := (v_total_cash_income + v_total_savings_cash + v_total_deposits_cash)
                    - (v_total_expenses_cash + v_total_withdrawals_cash + v_total_deposits_from_cash);

    -- Esewa: (Sales In + Savings In + Deposits In) - (Expenses Out + Withdrawals Out + Deposits From Esewa?)
    v_esewa_balance := (v_total_esewa_income + v_total_savings_esewa + v_total_deposits_esewa)
                     - (v_total_expenses_esewa);

    -- Fonepay: (Sales In + Savings In) - (Expenses Out)
    v_fonepay_balance := (v_total_fonepay_income + v_total_savings_fonepay)
                       - (v_total_expenses_fonepay);

    -- Cooperative: (Savings In) - (Withdrawals From Coop Out)
    ...
    -- 8. SPECIFIC REQUIREMENT for Verification System (Daily Analytical Cash Drift):
    -- Formula: (Cash Inflow) - (Cash Outflow)
    -- Inflow: Orders(Cash) + Charging(Cash) + Savings(Cash)
    -- Outflow: Expenses(Cash) + Withdrawals(Cash) + Deposits(From Cash)
    v_system_cash_calc := (v_total_income_cash_orders + v_total_income_cash_charging + v_total_savings_cash)
                        - (v_total_expenses_cash + v_total_withdrawals_cash + v_total_deposits_from_cash);
