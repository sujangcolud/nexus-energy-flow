-- Fix daily summary logic based on user clarification:
...
DECLARE
    ...
    -- Savings (User clarified: This is paying into a cooperative/bank from hand - Outflow)
    v_total_savings NUMERIC;
    v_total_savings_cash NUMERIC;
    v_total_savings_fonepay NUMERIC;
    v_total_savings_esewa NUMERIC;

    -- Withdrawals (Spending from Bank/Coop, if mode is Cash, it comes from hand?
    -- Or does it mean it's turned to cash and paid out? User says reductions.
    -- We treat Withdrawals with mode 'Cash' as outflows from hand.)
...
BEGIN
    ...
    -- 5. Calculate cooperative savings (User Clarification: Outflow from counter)
    SELECT
    ...
    -- Cash: (Inflows: Sales + DepositsToCash) - (Outflows: Exp + Sav + With + DepositsFromCash)
    v_cash_balance := (v_total_cash_income + v_total_deposits_cash)
                    - (v_total_expenses_cash + v_total_savings_cash + v_total_withdrawals_cash + v_total_deposits_from_cash);

    -- eSewa: (Inflows: Sales + DepositsToEsewa) - (Outflows: Exp + Sav)
    v_esewa_balance := (v_total_esewa_income + v_total_deposits_esewa)
                     - (v_total_expenses_esewa + v_total_savings_esewa);

    -- Fonepay: (Inflows: Sales) - (Outflows: Exp + Sav)
    ...
    -- (Cash In: Orders + Charging + DepositsToCash) - (Cash Out: Expenses + Savings + Withdrawals + DepositsFromCash)
...
DECLARE
    d DATE;
BEGIN
    FOR d IN SELECT DISTINCT summary_date FROM public.daily_summary LOOP
