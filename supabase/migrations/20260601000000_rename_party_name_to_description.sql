-- Migration to rename party_name to description and use supplier for party names in expense_bookings

DO $$
BEGIN
    -- Rename party_name to description if it exists and description doesn't
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='expense_bookings' AND column_name='party_name')
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='expense_bookings' AND column_name='description') THEN
        ALTER TABLE public.expense_bookings RENAME COLUMN party_name TO description;
    ELSIF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='expense_bookings' AND column_name='party_name')
       AND EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='expense_bookings' AND column_name='description') THEN
        -- If both exist, move data from party_name to description if description is null
        UPDATE public.expense_bookings SET description = party_name WHERE description IS NULL;
        ALTER TABLE public.expense_bookings DROP COLUMN party_name;
    END IF;
...
-- Update process_inventory_expense function to align with schema changes
