...
-- Re-verify and finalize update_daily_summary logic
-- Ensures all payment modes are correctly handled and formula signs are accounting-correct.
...
DECLARE
    ...
    -- Deposits (Money moved between wallets)
    v_total_deposits NUMERIC;
    v_total_deposits_cash NUMERIC; -- Money deposited TO Cash counter (Inflow)
    v_total_deposits_esewa NUMERIC;
    v_total_deposits_from_cash NUMERIC; -- Money deposited FROM Cash counter (Outflow)

    -- Savings (Member contributions - Inflow)
...
BEGIN
    ...
    -- 5. Calculate cooperative savings (Inflow)
    SELECT
    ...
    -- 6. Calculate withdrawals (Outflow)
    SELECT
        COALESCE(SUM(amount), 0),
    ...
    -- 7. Ledger Balances (Daily Net Change)
    -- Cash: (Inflows: Sales + Savings + DepositsTo) - (Outflows: Exp + With + DepositsFrom)
    ...
    -- eSewa: (Inflows: Sales + Savings + DepositsTo) - (Outflows: Exp + WithFromEsewa)
    v_esewa_balance := (v_total_esewa_income + v_total_savings_esewa + v_total_deposits_esewa)
                     - (v_total_expenses_esewa);

    -- Fonepay: (Inflows: Sales + Savings) - (Outflows: Exp)
    v_fonepay_balance := (v_total_fonepay_income + v_total_savings_fonepay)
                       - (v_total_expenses_fonepay);

    -- Cooperative: (Inflows: Savings) - (Outflows: WithdrawalsFromCoop)
    ...
    -- 8. Physical Cash Calculation (Verification Module):
    -- (Cash In: Orders + Charging + Savings + DepositsToCash) - (Cash Out: Expenses + Withdrawals + DepositsFromCash)
    v_system_cash_calc := (v_total_income_cash_orders + v_total_income_cash_charging + v_total_savings_cash + v_total_deposits_cash)
                        - (v_total_expenses_cash + v_total_withdrawals_cash + v_total_deposits_from_cash);
