-- Refine process_pos_order to strictly implement Batch Yield deduction logic
-- Formula: ((Ingredient Qty * (1 + Waste%)) / Recipe Yield) * Ordered Qty
...
BEGIN
  ...
  -- We use the Batch Yield logic here to correctly predict total usage
  WITH cart AS (
  ...
  needs AS (
    SELECT r.inventory_item_id,
           SUM(public.convert_unit(
             -- (Total Batch Quantity / Yield) * Number of servings sold
