...
serve(async (req) => {
  ...
  try {
    // Require authentication — this endpoint exposes business data
    ...
    if (question.length > 5000) {
      return new Response(
        JSON.stringify({ error: "Question exceeds 5000 character limit" }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    // Service-role client used internally to read aggregated business context
